...
const SECTIONS: LessonSection[] = [
  { id: "slides", label: "מצגת: הפרויקט המסכם של המודול", content: <SlideDeck slides={SLIDES} /> },
  {
    ...
    content: (
      <EngineeringInsights
        why="פרויקט זה קיים כדי לחבר תיאוריה (תבניות ארכיטקטורה, handoff) לקוד production אמיתי שרץ ב-AtlasDesk."
        alternatives="אפשר היה לממש את זה כ-tool calling רגיל (כמו tool-chat) בלי system prompt נפרד לחיוב — אבל זה בדיוק מפספס את היתרון של סוכן מומחה עם ידע ממוקד."
        whenNotTo="—"
        commonMistakes="לשכוח להעביר את מלוא ההיסטוריה לסוכן המומחה (כמו שהשיעור הקודם הזהיר) — בקוד הנוכחי, `messages` המלא מועבר לקריאה השנייה, לא רק ההודעה האחרונה."
        cost="שאלה שמסלימה עולה כפול (שתי קריאות Claude) — אבל זה קורה רק לשאלות חיוב מורכבות, לא לכל שיחה."
        realWorld="זו בדיוק הארכיטקטורה שמערכות תמיכה גדולות (עם 'רמות תמיכה' L1/L2/L3) משתמשות בה — נציג כללי שמסלים למומחה כשצריך."
      />
    ),
  },
  {
